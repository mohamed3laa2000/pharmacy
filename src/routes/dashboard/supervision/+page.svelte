<script>
	...
	async function loadMedicines(initial = true) {
		...
		let q = query(
			...
			where('category', '==', 'الأدوية الخاضعة للرقابة'),
			orderBy('name'),
			limit(pageSize)
		);
	...
	let newMedicine = { name: '', category: 'الأدوية الخاضعة للرقابة', unit: '' };
	...
	async function addMedicine() {
		...
		newMedicine = { name: '', category: 'الأدوية الخاضعة للرقابة', unit: '' };
