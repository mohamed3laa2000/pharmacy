<script>
	...
	async function loadMedicines(initial = true) {
		...
		let q = query(
			...
			where('category', '==', 'المستلزمات الطبية'),
			orderBy('name'),
			limit(pageSize)
		);
	...
	let newMedicine = { name: '', category: 'المستلزمات الطبية', unit: '' };
	...
	async function addMedicine() {
		...
		newMedicine = { name: '', category: 'المستلزمات الطبية', unit: '' };
